...
class GameSerializerTestCase(TestCase):
      ...
      def test_serializer_blank_fields(self):
        serializer = GamesSerializer(data={'date':'', 'is_finished':'', 'stadium':'', 'home_team':{'name':'', 'group':'', 'is_eliminated':''}, 'away_team':{'name':'', 'group':'', 'is_eliminated':''}, 'goal_home_team':'', 'goal_away_team':''})
        self.assertFalse(serializer.is_valid())
        ...
        self.assertEqual(serializer.errors.get("home_team")["name"][0], "This field may not be blank.")
        self.assertEqual(serializer.errors.get("away_team")['name'][0], "This field may not be blank.")
      ...
      def test_invalid_fields(self):
        game1 = GamesFactory()
        game2 = GamesFactory()
        serializer = GamesSerializer(data={'date':'27/12/2022', 'stadium':'randomstadium', 'home_team':{'name':'Test','group':'Z','is_eliminated':'not_boolean'}, 'away_team':{'name':'Test2', 'group':'X', 'is_eliminated':'not_boolean'}, 'goal_home_team':'XX', 'goal_away_team':'YY'})
        self.assertFalse(serializer.is_valid())
        self.assertEqual(serializer.errors.get("date")[0].code, "invalid")
        self.assertEqual(serializer.errors.get("home_team")["group"][0], '"Z" is not a valid choice.')
        self.assertEqual(serializer.errors.get("home_team")["is_eliminated"][0], "Must be a valid boolean.")
        self.assertEqual(serializer.errors.get("away_team")["group"][0], '"X" is not a valid choice.')
        self.assertEqual(serializer.errors.get("away_team")["is_eliminated"][0], "Must be a valid boolean.")
        self.assertEqual(serializer.errors.get("goal_home_team")[0].code, "invalid")
        self.assertEqual(serializer.errors.get("goal_away_team")[0].code, "invalid")
